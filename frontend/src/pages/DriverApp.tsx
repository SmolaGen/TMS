...
export const DriverApp: React.FC = () => {
  ...
  const { latitude, longitude, error, isTracking, startTracking, stopTracking } = useGeoTracking(
    user?.driver_id,
  );
  ...
  return (
    <Layout style={{ minHeight: '100vh', background: 'var(--tms-bg-layout)' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
          <Badge
            status={isTracking ? 'success' : 'default'}
            text={isTracking ? 'Online' : 'Offline'}
          />

          {isDevMode() && (
            <Button type="primary" ghost icon={<SwapOutlined />} onClick={switchRole}>
              Диспетчер
            </Button>
          )}
      ...
      <Content style={{ padding: '20px' }}>
        <Space orientation="vertical" size="large" style={{ width: '100%' }}>
          ...
          <Card size="small" style={{ borderRadius: '12px' }}>
            <Space align="center">
              <EnvironmentOutlined
                style={{ fontSize: '24px', color: isTracking ? '#52c41a' : '#bfbfbf' }}
              />
              <div>
                <Text strong style={{ display: 'block' }}>
                  {isTracking ? 'Геолокация активна' : 'Геолокация выключена'}
                </Text>
                <Text type="secondary" style={{ fontSize: '12px' }}>
                  {latitude && longitude
                    ...
                    : error ||
                      (isTracking
                        ? 'Ожидание координат...'
                        : 'Нажмите "Начать смену" для активации')}
                </Text>
              </div>
            </Space>
          </Card>
                ...
                >
                  <Space orientation="vertical" style={{ width: '100%' }}>
                    ...
                    <Space size="small">
                      ...
                      <Text type="secondary">
                        {order.time_start
                          ? new Date(order.time_start).toLocaleTimeString([], {
                              hour: '2-digit',
                              minute: '2-digit',
                            })
                          : '--:--'}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                      ...
                      <Text style={{ fontSize: '13px' }}>
                        ...
                        {order.dropoff_address || 'Не указан'}
                      </Text>
                    </div>
                  </Space>
                </Card>
