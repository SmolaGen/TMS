      ...
      <Space direction="vertical" style={{ width: '100%' }} size="small">
        ...
        <Space>
          <Badge status={config.badge} dot offset={[-4, 30]}>
            <Avatar size={48} icon={<UserOutlined />} style={{ backgroundColor: config.color }} />
          </Badge>
          <div>
        ...
        <Space>
          ...
          <Text type="secondary">{dayjs(driver.updated_at).fromNow()}</Text>
        </Space>

        {/* Статус активности */}
        {!driver.is_active && <Tag color="red">Деактивирован</Tag>}
      </Space>
    </Card>
  );
};
