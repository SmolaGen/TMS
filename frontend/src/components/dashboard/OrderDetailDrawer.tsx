...
export const OrderDetailDrawer: React.FC<OrderDetailDrawerProps> = ({
  ...
  const handleAction = (
    action: OrderAction,
    params: { reason?: string; driverId?: number } = {},
  ) => {
    if (!orderId) return;
        ...
        <>
          ...
          <Descriptions title="Информация" column={1} layout="vertical" bordered size="small">
            <Descriptions.Item
              label={
                <>
                  <EnvironmentOutlined /> Откуда
                </>
              }
            >
              <Text strong>
                {order.pickup_address || `${order.pickup_lat}, ${order.pickup_lon}`}
              </Text>
            </Descriptions.Item>
            <Descriptions.Item
              label={
                <>
                  <EnvironmentOutlined /> Куда
                </>
              }
            >
              <Text strong>
                {order.dropoff_address || `${order.dropoff_lat}, ${order.dropoff_lon}`}
              </Text>
            </Descriptions.Item>
            <Descriptions.Item
              label={
            ...
            >
              {dayjs(order.time_start).format('DD.MM HH:mm')} —{' '}
              {dayjs(order.time_end).format('HH:mm')}
            </Descriptions.Item>
            <Descriptions.Item
              label={
                <>
