...
export const Dashboard: React.FC = () => {
  ...
  const timelineDrivers: TimelineDriver[] = useMemo(() => {
    ...
    return [{ id: 'unassigned', content: '❌ Не назначено' }, ...transformed];
  }, [drivers]);
  ...
  return (
    ...
    >
      ...
      <Card size="small" style={{ marginBottom: 0 }}>
        <Space>
          ...
          <DatePicker
            ...
            onChange={(val) => {
              if (val) {
                setDateRange([val.startOf('day').toDate(), val.endOf('day').toDate()]);
              }
            }}
            allowClear={false}
          />
          <Button icon={<ImportOutlined />} onClick={() => setIsExcelModalOpen(true)}>
            Импорт Excel
          </Button>
        </Space>
      </Card>
      ...
      <ExcelImportModal open={isExcelModalOpen} onCancel={() => setIsExcelModalOpen(false)} />
    </div>
  );
};
