...
monitor_progress() {
  ...
  while true; do
    ...
    local secs=$((elapsed % 60))
        ...
        # Reliable stream reading using file offset logic (simulated with diff)

        if [[ -f "$file" ]] && [[ -s "$file" ]]; then

          

          # 1. Extract and print NEW text messages

          # We grep for text events, extract the message content, and filter out what we've already seen

          # Using a simpler approach: just get the very last message and print it if it changed

          # (Full streaming in pure bash is tricky without proper offset tracking, so we stick to 'latest status' approach but make it robust)

          

          local latest_text_full

          # Support both standard Claude and Gemini/VibeProxy JSON structure

          latest_text_full=$(tail -c 10000 "$file" 2>/dev/null | grep '"type":"text"' | tail -n 1)

          

          local latest_msg_content=""

          if [[ -n "$latest_text_full" ]]; then

             # Try to extract content

             latest_msg_content=$(echo "$latest_text_full" | jq -r '.part.text // .message.content[0].text // ""' 2>/dev/null)

          fi

    

          if [[ -n "$latest_msg_content" ]] && [[ "$latest_msg_content" != "$last_seen_msg" ]]; then

             tput cr 2>/dev/null || printf "\r"

             tput el 2>/dev/null || true

             echo "  ${CYAN}🤖 ${latest_msg_content}${RESET}"

             last_seen_msg="$latest_msg_content"

          fi

    

          # 2. Update status line (activeForm)

          local active_form

          active_form=$(tail -c 5000 "$file" 2>/dev/null | grep -o '"activeForm":"[^"]*"' | tail -1 | cut -d'"' -f4)
          ...
          # Print without newline to keep it at the bottom

          printf "  ${MAGENTA}%s [%02d:%02d] %-25s${RESET}" "$spinner_char" "$mins" "$secs" "${current_step:0:25}"
          ...
          printf "  ${BLUE}⠋ [%02d:%02d] Waiting for AI...${RESET}" "$mins" "$secs"
...
run_ai_command() {
  ...
  case "$AI_ENGINE" in
    ...
    codex)
      ...
      ;;
    *)
      # Claude Code via VibeProxy (Gemini Flash)
      env ANTHROPIC_BASE_URL="http://localhost:8317" \
          ANTHROPIC_API_KEY="sk-vibeproxy-dummy" \
          ANTHROPIC_MODEL="gemini-3-flash-preview" \
          claude --dangerously-skip-permissions \
...
# Run main
main "$@"
 | tail -n 1)$' | tail -n 1)      # Extract active form (current tool/action)
      local active_form
      active_form=$(tail -c 5000 "$file" 2>/dev/null | grep -o '"activeForm":"[^"]*"' | tail -1 | cut -d'"' -f4)
      ...
      if [[ "$new_text_lines" != "$last_text_seen" ]] && [[ -n "$new_text_lines" ]]; then
         tput cr 2>/dev/null || printf "\r"
         tput el 2>/dev/null || true
         echo "  ${CYAN}🤖 ${new_text_lines}${RESET}"
         last_text_seen="$new_text_lines"
      fi
      
      # Print status line at the bottom (overwrite)
      local spinner_char="${spinstr:$spin_idx:1}"
      tput cr 2>/dev/null || printf "\r"
      printf "  ${MAGENTA}%s [%02d:%02d] %-20s${RESET}" "$spinner_char" "$mins" "$secs" "${current_step:0:20}"
    else
      tput cr 2>/dev/null || printf "\r"
      printf "  ${BLUE}⠋ [%02d:%02d] Waiting for AI...${RESET}" "$mins" "$secs"
    fi
...
# Variable to track last seen text needs to be initialized before loop
last_text_seen=""
