...
## 5. Паттерны реализации и устранения ошибок (Lessons Learned)
*   **Asyncpg Range Types**: Драйвер `asyncpg` не поддерживает приведение строк к `tstzrange` (`:val::tstzrange`). Всегда используй SQL-конструктор: `tstzrange(:start, :end, '[)')`.
*   **Изоляция тестов (Nested Transactions)**: Для тестов, проверяющих ошибки БД (например, IntegrityError), используй `await session.begin_nested()` (SAVEPOINT), чтобы ошибка не ломала основную транзакцию теста.
*   **Safe Error Handling**: При обработке `IntegrityError` (после `commit()`) сессия становится невалидной ("rolled back"). Не обращайся к ленивым атрибутам объектов (например, `order.driver_id`). Кэшируй их в локальные переменные **до** коммита.
*   **Порядок роутов**: Статические пути (например, `/orders/active`) всегда должны быть определены **выше**, чем динамические (`/orders/{id}`), чтобы избежать `422 Unprocessable Entity` (валидация int).
